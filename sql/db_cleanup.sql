...
begin
  if not exists (
    ...
    execute 'alter table public.messages add column user_uuid uuid';
  end if;
end $$;
  ...
  if exists (
    ...
    execute $sql$
      ...
      using case
        ...
        when trim(user_uuid::text) = '' then null
        when trim(user_uuid::text) ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'
          then trim(user_uuid::text)::uuid
        else null
      end
    $sql$;
  end if;
...
begin
  if exists (
    ...
    execute 'alter table public.messages add constraint messages_user_uuid_fkey foreign key (user_uuid) references auth.users(id) on delete set null';
  end if;
end $$;
...
create index if not exists messages_user_uuid_idx on public.messages (user_uuid);
...
-- 5) Push subscriptions for web push delivery
